...
function getMiddle(s){
  const splitedNum = s.split("");
  if (splitedNum.length === 1 || splitedNum.length === 2) {
    const joinedArr = splitedNum.join("");
    return joinedArr;
  }
  for (i = 0; 1 < i < splitedNum.length; i++) {
    if (splitedNum.length === 1 || splitedNum.length === 2) {
      const joiningArr = splitedNum.join("");
      return joiningArr;
    }
      splitedNum.pop();
    splitedNum.shift();
